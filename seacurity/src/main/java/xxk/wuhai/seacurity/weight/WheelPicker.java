...
import xxk.wuhai.seacurity.utils.LinearGradient;
...
public class WheelPicker<T> extends View {
	...
	private void computeFlingLimitY() {
        mMinFlingY = mIsCyclic ? Integer.MIN_VALUE :
				- mItemHeight * (mDataList == null?0:mDataList.size() - 1);
    ...
    private int fixItemPosition(int position) {
        if (position < 0) {
            ...
            position = mDataList.size() + (position % (mDataList.size()==0?1:mDataList.size()));

        }
        if (position >= mDataList.size()){
            //将数据集限定在0 ~ mDataList.size()-1之间
            position = position % (mDataList.size() == 0?1:mDataList.size());
        }
        return position;
    }
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1;
            ...
            if (mIsCyclic) {
				...
				if (position < 0 || position > (mDataList == null?0:mDataList.size() - 1)) {
					continue;
				}
			}
